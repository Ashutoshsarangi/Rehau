...
const isIpad = (win: Window) => testUserAgent(win, /iPad/i);

const isIphone = (win: Window) => testUserAgent(win, /iPhone/i);

const isIOS = (win: Window) => testUserAgent(win, /iPad|iPhone|iPod/i);
...
const isBrowser = (win: Window) => !isElectron(win) && (isMobileWeb(win) || !isMobile(win));
